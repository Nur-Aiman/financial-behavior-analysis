...
export class FixedExpenseController {
  ...
  static async getAll(_req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const enriched = payments.map(p => ({
        ...
        actualAmount: p.actualAmountCents ? formatCentsAsRinggit(p.actualAmountCents) : null,
      }));

      res.json(successResponse(enriched, 'Fixed expenses retrieved'));
    } catch (err) {
      next(err);
    }
  }
   ...
   */
  static async getUnpaid(_req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      res.json(successResponse(enriched, 'Unpaid expenses retrieved'));
    } catch (err) {
      next(err);
    }
  }
  ...
  static async getOverdue(_req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      res.json(successResponse(enriched, 'Overdue expenses retrieved'));
    } catch (err) {
      next(err);
    }
  }
   ...
   * Pay fixed expense
   */
  static async payExpense(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
   ...
   */
  static async reversePayment(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      fixedExpenseService.reverseFixedExpensePayment(req.params.categoryId);
      res.json(successResponse({ categoryId: req.params.categoryId }, 'Payment reversed'));
    } catch (err) {
      next(err);
    }
  }
