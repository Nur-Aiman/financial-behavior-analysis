...
export class BalanceService {
   ...
   */
  getAdjustmentHistory(): BalanceAdjustment[] {
   ...
   */
  async updateBalance(newBalanceCents: number, reason: string): Promise<BalanceAdjustment> {
   ...
   * Add income
   */
  async addIncome(amountCents: number, description?: string): Promise<void> {
    if (amountCents <= 0) {
      throw new AppError({
        code: 'INVALID_BALANCE_AMOUNT',
   ...
   * Deduct from balance
   */
  async deductFromBalance(amountCents: number): Promise<void> {
    if (amountCents <= 0) {
      throw new AppError({
        code: 'INVALID_BALANCE_AMOUNT',
